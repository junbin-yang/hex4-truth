...
通用安全监控器组件(设计见 [`../../../docs/ESP32-S3安全监控器设计文档.md`](../../../docs/ESP32-S3安全监控器设计文档.md) v1.0)。
上位机经 UART 下发指令帧(演示工程默认 UART0/USB-UART 口, 部署形态改 UART1),监控器完成确定性判定后**在返回判定回执前**执行或拒绝;
...
| `guard_uart.h/.c` | UART 适配(事件任务收帧、断线检测、残帧超时; 演示 UART0, 部署 UART1) | ✅ |
| `guard_permissions.h/.c` | 动作表/权限表/参数域(使用方配置,固化 flash) | ✅ |
| `guard_verify.h/.c` | 角色验签(逐密钥 HMAC 规范编码) | ✅ |
| `guard_policy.h/.c` | L3 判定(权限掩码 + 参数形状 RANGE/ENUM/RANGE_LUT/COND + 动作门控) | ✅ |
| `guard_state.h/.c` | 安全状态机(表驱动转移/deny 位图/E-STOP 确认重启闭环) | ✅ |
...
```bash
cd host_tests && make test   # 220 项: 帧协议 94 + 判定链 85 + 状态机 41
```
...
| 项 | 值 |
|---|---|
| 设备侧判定耗时 (diag_us) | 877~1338 µs (JSON+HMAC+L3+L4+回执) |
